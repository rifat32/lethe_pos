@foreach( $variations as $variation)
    <tr>
        ...
        <td>
            {{ $product->name }} ({{$variation->sub_sku}}) <br/>(Stock:
            <?php
            if (isset($vld[0])) {
                echo $vld[0]->qty_available;
            }else{
                echo '0';
            }
    ?>
        ...
        <td>
                @if(!empty($allowed_group_prices))
                        @foreach($allowed_group_prices as $key => $value)
                            ...
                            <br>
                        @endforeach
                    {{--<input value="120.00" class="form-control input-sm input_number" required="" name="purchases[1][default_sell_price]" type="text" >--}}
                    @else
                    No Permission
                @endif
        </td>
        ...
        <td hidden>
        <input type="text" name="mrp_price" value="{{$product->mrp_price}}" class="form-control input-sm input_number default_sell_price">
        </td>
        <td hidden>
        <input type="text" name="reseller_price" value="{{$product->reseller_price}}" class="form-control input-sm input_number default_sell_price">
        </td>
        ...
        @if(session('business.enable_product_expiry'))
            <td style="text-align: left;">
                ...
                <div class="input-group">
                    ...
                    --}}
                    <!-----------------Moinul------------------------->
                    {!! Form::text('purchases[' . $row_count . '][exp_date]', null, ['class' => 'form-control input-sm expiry_datepicker exp_date']); !!} 
                      <!-----------------Moinul------------------------->
                </div>
                @else
