<tr class="product_row" data-row_index="{{$row_count}}">
	...
	<td class="text-center v-center">
      ...
      $sis_price=($product->quantity_ordered*$unit_price_inc_tax);
		@endphp
		...
		<span class="display_currency pos_line_total_text @if(!empty($pos_settings['is_pos_subtotal_editable'])) hide @endif" data-currency_symbol="true">{{$sis_price}}</span>
	</td>
	@php
		    $product_category_id = \App\Product::where([
	...
	@endphp
    <td>
		<input type="text" class="form-control" name="products[{{$row_count}}][cost]" value="{{@num_format($product->cost)}}">
        <input type="hidden" class="form-control" name="products[{{$row_count}}][category]" value="{{($product_category_id)}}">
		<input type="hidden" class="form-control" name="products[{{$row_count}}][assistant]" value="yes">
