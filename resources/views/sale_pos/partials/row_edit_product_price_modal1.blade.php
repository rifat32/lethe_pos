	...
	<div class="modal-content">
		...
		<div class="modal-body">
			<div class="row">
				...
				@php
					$discount_type = !empty($product->line_discount_type) ? $discount->discount_type : 'fixed';
					$discount_amount = !empty($product->line_discount_amount) ? $discount->discount_amount : 0;
				@endphp
				<div class="form-group col-xs-12 col-sm-6 @if(!auth()->user()->can('edit_product_discount_from_sale_screen')) hide @endif">
					<label>@lang('sale.discount_type')</label>
						{!! Form::select("products[$row_count][line_discount_type]", ['fixed' => __('lang_v1.fixed'), 'percentage' => __('lang_v1.percentage')], $discount->discount_type , ['class' => 'form-control row_discount_type']); !!}
				</div>
				<div class="form-group col-xs-12 col-sm-6 @if(!auth()->user()->can('edit_product_discount_from_sale_screen')) hide @endif">
					<label>@lang('sale.discount_amount')</label>
						{!! Form::text("products[$row_count][line_discount_amount]", @num_format($discount->discount_amount), ['class' => 'form-control input_number row_discount_amount']); !!}
				</div>
