        ...
        <div class="table-responsive">
          <table class="table bg-gray">
            ...
            @foreach($sell->sell_lines as $sell_line)
                @php
                    if($sell_line->service_center_id !=null){
                        $bar=DB::table('bar')->select('bar_name')->where('id',$sell_line->service_center_id)->first();
                    }
                @endphp
              <tr>
                ...
                <td><span class="display_currency" data-currency_symbol="false">{{ $sell_line->quantity }}</span></td>
                <td>
            ...
            <tr>
              ...
              <td>
                  <span class="display_currency pull-right" data-currency_symbol="true">
                      {{-- $sell->total_before_tax --}} {{-- $sell->final_total --}}
                    @php
                        $withOutDiscount = filter_var($total_paid, FILTER_SANITIZE_NUMBER_FLOAT, FILTER_FLAG_ALLOW_FRACTION);

                        $dscunt = filter_var($sell->discount_amount, FILTER_SANITIZE_NUMBER_FLOAT, FILTER_FLAG_ALLOW_FRACTION);

            if ($sell->discount_type == 'fixed') {
                $discount = $dscunt;
            } else {
                $discount  = ($dscunt/100)*$sell->total_before_tax;
            }

                        //$totalAmount = $withOutDiscount + $dscunt;
                        $totalAmount = $withOutDiscount;
                    @endphp
                         {{-- number_format((float)$totalAmount, 2, '.', '')--}}

                         {{ $sell->final_total + $discount}}
                  </span>
              </td>
            </tr>
            <tr>
