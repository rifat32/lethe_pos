$(document).ready( function () {
    ...
    stock_adjustment_table = $('#stock_adjustment_table').DataTable({
		...
		columns: [
            ...
            { data: 'supplier_name', name: 'CN.name'},
    ...
    $('#stock_adjustment_table tbody').on( 'click', '.view_stock_return', function () {
    ...
    stock_adjustment_table.on( 'draw', function () {
        $.each( detailRows, function ( i, id ) {
            $('#'+id+' .view_stock_return').trigger( 'click' );
        } );
    } );
...
function get_stock_adjustment_details(rowData){
	var div = $('<div/>')
        ...
        .text( 'Loading...' );
    $.ajax( {
        url: '/stock-return/' + rowData.DT_RowId,
        dataType: 'html',
...
$(document).on('click', 'button.print_stock_return', function(e){
		e.preventDefault();
        var href = $(this).data('href');
