$(document).ready(function () {
    ...
    $("#doctor_id").select2({
		...
		language: {
			noResults: function () {
				var name = $("#doctor_id").data("select2").dropdown.$search.val();
	...
	$("#assistant_id").select2({
		...
		language: {
			noResults: function () {
				var name = $("#assistant_id").data("select2").dropdown.$search.val();
    ...
    $("#service_id").select2({
		...
		language: {
			noResults: function () {
				var name = $("#service_id").data("select2").dropdown.$search.val();
	...
	$.ajax({
			method: "GET",
			url: `/contacts/single/${e.params.data.id}`,
			dataType: "json",
			success: function (data) {
			let sale_due =
					parseFloat(data.total_invoice) -
					parseFloat(data.invoice_received);
				let sale_due_message = ` total sale due ৳ ${parseFloat(sale_due)}`;

				let sale_due_return =
					parseFloat(data.total_sell_return) -
					parseFloat(data.sell_return_paid);
				let sale_due_return_message = ` total sale return due ৳ ${parseFloat(
					sale_due_return
				)}`;

				document.getElementById("sell_due").innerHTML = sale_due_message;

				document.getElementById("sell_return_due").innerHTML =
					sale_due_return_message;
		...
		.autocomplete({
			...
			response: function (event, ui) {
				if (ui.content.length == 1) {
					...
					swal(LANG.no_products_found).then((value) => {
    ...
    .autocomplete({
        source: function (request, response) {
			...
			if(!doctor_id){
				toastr.warning("please select doctor first")
				return;
			}
            if ($("#price_group").length > 0) {
                price_group = $("#price_group").val();
    ...
    .autocomplete({
        source: function (request, response) {
            ...
            $.getJSON(
                "/products/list/service/assistant",
                {
                    price_group: price_group,
                    location_id: $("input#location_id").val(),
                    term: request.term,
        ...
        select: function (event, ui) {
            if (ui.item.enable_stock != 1 || ui.item.qty_available > 0) {
                $(this).val(null);
                pos_product_row(ui.item.variation_id,true);
            } else {
                alert(LANG.out_of_stock);
            }
        },
	...
	$("table#pos_table tbody").on("change", "input.pos_quantity", function () {
    	...
    	//Update line total and check for quantity not greater than max quantity
	$("table#pos_table2 tbody").on("change", "input.pos_quantity", function () {
		if (sell_form_validator) {
			sell_form_validator.element($(this));
		}
		if (pos_form_validator) {
		...
		pos_total_row();
	  	...
	  	//Update line total and check for quantity not greater than max quantity
		  $("table#pos_table3 tbody").on("change", "input.pos_quantity", function () {
			if (sell_form_validator) {
				sell_form_validator.element($(this));
			}
			if (pos_form_validator) {
    ...
    //If change in unit price update price including tax and line total
	$("table#pos_table2 tbody").on("change", "input.pos_unit_price", function () {
		var unit_price = __read_number($(this));
		var tr = $(this).parents("tr");
	  ...
	  //If change in unit price update price including tax and line total
	  $("table#pos_table3 tbody").on("change", "input.pos_unit_price", function () {
		var unit_price = __read_number($(this));
		var tr = $(this).parents("tr");
	...
	//If change in tax rate then update unit price according to it.
	$("table#pos_table2 tbody").on("change", "select.tax_id", function () {
		var tr = $(this).parents("tr");
	...
	//If change in tax rate then update unit price according to it.
	$("table#pos_table3 tbody").on("change", "select.tax_id", function () {
		var tr = $(this).parents("tr");
...
//Change max quantity rule if lot number changes
$("table#pos_table2 tbody").on("change", "select.lot_number", function () {
    var qty_element = $(this).closest("tr").find("input.pos_quantity");
    if ($(this).val()) {
        var lot_qty = $("option:selected", $(this)).data("qty_available");
        var max_err_msg = $("option:selected", $(this)).data("msg-max");
...
//Change max quantity rule if lot number changes
$("table#pos_table3 tbody").on("change", "select.lot_number", function () {
    var qty_element = $(this).closest("tr").find("input.pos_quantity");
    if ($(this).val()) {
        var lot_qty = $("option:selected", $(this)).data("qty_available");
        var max_err_msg = $("option:selected", $(this)).data("msg-max");
	...
	//Remove row on click on remove row
	$("table#pos_table2 tbody").on("click", "i.pos_remove_row", function () {
		swal({
			title: LANG.sure,
			icon: "warning",
			buttons: true,
	...
	//Remove row on click on remove row
	$("table#pos_table3 tbody").on("click", "i.pos_remove_row", function () {
		swal({
			title: LANG.sure,
			icon: "warning",
			buttons: true,
	...
	$("button#pos-draft").click(function () {
		...
		if ($("table#pos_table tbody").find(".product_row").length <= 0 && $("table#pos_table2 tbody").find(".product_row").length <= 0  && $("table#pos_table3 tbody").find(".product_row").length <= 0) {
	...
	$("button#pos-quotation").click(function () {
		//Check if product is present or not.
		if ($("table#pos_table tbody").find(".product_row").length <= 0 && $("table#pos_table2 tbody").find(".product_row").length <= 0
		&& $("table#pos_table3 tbody").find(".product_row").length <= 0
		) {
			toastr.warning(LANG.no_products_added);
			return false;
		}
	...
	$(".register_details_modal, .close_register_modal").on(
		"shown.bs.modal",
		function () {
	...
	// 	null
	// );
	$("select#product_category, select#product_brand, select#doctor_id, select#assistant_id").on(
		"change",
		function (e) {
	...
	$("#search_product").keydown(function (e) {
		...
		if (key == 9) {
            ...
            if ($("#pos_table2 tbody tr").length > 0) {
				$("#pos_table2 tbody tr:last")
					.find("input.pos_quantity")
					.focus()
					.select();
			}
			if ($("#pos_table3 tbody tr").length > 0) {
				$("#pos_table3 tbody tr:last")
					.find("input.pos_quantity")
					.focus()
					.select();
			}
	...
	$("#pos_table2").on("keypress", "input.pos_quantity", function (e) {
		var key = e.which;
		if (key == 13) {
			// the enter key code
			$("#search_product").focus();
	...
	$("#pos_table3").on("keypress", "input.pos_quantity", function (e) {
		var key = e.which;
		if (key == 13) {
			// the enter key code
			$("#search_product").focus();
	...
	$("select#price_group").change(function () {
		...
		if (curr_val != prev_value && ($("table#pos_table tbody tr").length > 0 || $("table#pos_table2 tbody tr").length > 0 || $("table#pos_table3 tbody tr").length > 0)) {
			swal({
				title: LANG.sure,
				...
				if (willDelete) {
					if ($("form#edit_pos_sell_form").length > 0) {
						...
						pos_total_row();
						if ($("form#edit_pos_sell_form").length > 0) {
							$("table#pos_table2 tbody").html("");
							pos_total_row();
						}
						if ($("form#edit_pos_sell_form").length > 0) {
							$("table#pos_table3 tbody").html("");
							pos_total_row();
						}
					}
...
function get_product_suggestion_list(
	...
	url = null,
     ...
     let doctor_id = 	$("select#doctor_id").val()
	 let assistant_id = 	$("select#assistant_id").val()
	if (url == null) {
...
function pos_product_row(variation_id,assistant = false) {
		...
		$("#pos_table tbody")
			...
			.each(function () {
				...
				if (
					...
					$("input#search_product2").focus().select();
				}
			});
			$("#pos_table3 tbody")
			.find("tr")
			.each(function () {
				...
				if (
					...
					$("input#search_product3").focus().select();
				}
			});
...
if(assistant){
	$.ajax({
		method: "GET",
		url: "/sells/pos/get_product_row/assistant/" + variation_id + "/" + location_id,
		async: false,
		data: {
		...
		success: function (result) {
			if (result.success) {
			   ...
			   if(result.product_category_id == "30"){
			
				console.log(result.html_content)
		
				$("table#pos_table3 tbody")
				.prepend(result.html_content)
				.find("input.pos_quantity");
				
			//increment row count
			$("input#product_row_count").val(parseInt(product_row) + 1);
			var this_row = $("table#pos_table3 tbody").find("tr").first();
		
			pos_each_row(this_row);
			pos_total_row();
			if (result.enable_sr_no == "1") {
				var new_row = $("table#pos_table3 tbody").find("tr").last();
		...
		success: function (result) {
			if (result.success) {
			   ...
			   if(result.product_category_id == "30"){
				...
				$("table#pos_table2 tbody")
				.prepend(result.html_content)
				.find("input.pos_quantity");
				
			//increment row count
			$("input#product_row_count").val(parseInt(product_row) + 1);
			var this_row = $("table#pos_table2 tbody").find("tr").first();
		
			pos_each_row(this_row);
			pos_total_row();
			if (result.enable_sr_no == "1") {
				var new_row = $("table#pos_table2 tbody").find("tr").last();
				new_row.find(".add-pos-row-description").trigger("click");
			}
...
function pos_total_row() {
	...
	$("table#pos_table2 tbody tr").each(function () {
		total_quantity =
			total_quantity + __read_number($(this).find("input.pos_quantity"));
		price_total =
			price_total + __read_number($(this).find("input.pos_line_total"));
		...
		$("table#pos_table3 tbody tr").each(function () {
		total_quantity =
			total_quantity + __read_number($(this).find("input.pos_quantity"));
		price_total =
			price_total + __read_number($(this).find("input.pos_line_total"));
...
function reset_pos_form() {
	...
	if ($("form#edit_pos_sell_form").length > 0) {
		setTimeout(function () {
			// window.location = "/pos/pos/create/";
		}, 4000);
		return true;
	}
...
function set_default_customer() {
	...
	var exists = $(
		"#customer_id option[value=" + default_customer_id + "]"
	).length;
	if (exists == 0) {
		$("select#customer_id").append(
			$("<option>", {
...
//Update quantity if line subtotal changes
$("table#pos_table2 tbody").on("change", "input.pos_line_total", function () {
	var subtotal = __read_number($(this));
	var tr = $(this).parents("tr");
	var quantity_element = tr.find("input.pos_quantity");
	var unit_price_inc_tax = __read_number(
