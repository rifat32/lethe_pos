...
function pos_product_row(variation_id){
	...
	if(add_via_ajax){
		...
		var unit=$('select#unit_select_id').find('option:selected').val();
		...
		$.ajax({
			...
			success: function(result){
				if(result.success){
					$('table#pos_table tbody').prepend(result.html_content).find('input.pos_quantity');
					//increment row count
...
function set_default_customer(){
	...
	var exists  = $('#customer_id option[value='+default_customer_id+']').length;
	if ( exists == 0 ) {
