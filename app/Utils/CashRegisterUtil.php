...
class CashRegisterUtil extends Util
{
    ...
    public function addDueSellPaid($transaction, $payment){
    ...
    public function getRegisterDetails($register_id = null){
        ...
        if (empty($register_id)) {
            $user_id = auth()->user()->id;
            $query->where('cash_registers.user_id', $user_id)
    ...
    public function getRegisterTransactionSellReturn($user_id, $open_time, $close_time){
        return  Transaction::where('transactions.created_by', $user_id)
