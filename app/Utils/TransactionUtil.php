...
"order_id"=> !empty($input["order_id"])?$input["order_id"]:null,
            'type' => 'sell',
            'status' => $input['status'],
     ...
     */
    public function createOrUpdateSellLines($transaction, $products,$contact_id, $location_id, $return_deleted = false, $status_before = null, $extra_line_parameters = [],$input=null)
    {

         $doctor_id = !empty($input["doctor_id"])?$input["doctor_id"]:NULL;
         $assistant_id = !empty($input["assistant_id"])?$input["assistant_id"]:NULL;
 ...
 $commission =   DoctorCommission::where([
    ...
    if($commission && empty($product['assistant'])){
      $doctor_commission = $commission->doctor_commission;
    }
            ...
            if (!empty($product['transaction_sell_lines_id'])) {
                            ...
                            if (!empty($product['modifier_sell_line_id'][$key])) {
                                ...
                                if (!empty($product['modifier_price'][$key])) {
                                    ...
                                    $modifiers_formatted[] = new TransactionSellLine([
                                        'assistant_id' => !empty($product['assistant'])?$assistant_id:null,
                    "doctor_id"=> $doctor_commission > 0?$doctor_id:null,
                ...
                //code for insert item into discount table


              if($discount_amount>0)
              {
                $find_discount=DB::Table('discount')
                ...
                if($find_discount)
                {
                    ...
                    ->update(['discount_type'=>$product['line_discount_type'],'discount_amount'=>$discount_amount,'unit_price'=>$unit_price]);


                }
                else{
                     DB::Table('discount')->insert(['customer_id' =>$contact_id,'product_id' =>$product['product_id'],
                 'discount_type' =>$product['line_discount_type'],'discount_amount'=>$discount_amount,'unit_price'=>$unit_price]);
                }
              }
    ...
    public function createOrUpdatePaymentLines_bank($transaction, $payments, $business_id = null, $user_id = null)
    {
        $payments_formatted = [];
        $edit_ids = [0];
        ...
        foreach ($payments as $payment) {
            ...
            if (!empty($payment['payment_id'])) {
                ...
                if ($this->num_uf($payment['amount']) != 0) {
                    ...
                    ///code for bank tax---
                    if(isset($payment['bank_name'])){
                        if($payment_data['bank_name']=="sk")
                        {
                          $payment_data['tax_amount']=  ($payment_data['amount']*3)/100;
                          $payment_data['remain_amount']= $payment_data['amount'] -$payment_data['tax_amount'];
                          //echo $amount;

                        }
                        else
                        {
                         $payment_data['tax_amount']=  ($payment_data['amount']*2)/100;
                         $payment_data['remain_amount']= $payment_data['amount'] -$payment_data['tax_amount'];
    ...
    public function getReceiptDetails($transaction_id, $location_id, $invoice_layout, $business_details, $location_details, $receipt_printer_type)
    {
        ...
        if ($il->show_mobile_number == 1 && !empty($location_details->mobile)) {
            if($location_details->alternate_number) {
                $output['contact'] .= 'Mobile: ' . $location_details->mobile . ", " . $location_details->alternate_number;
            } else {
    ...
    public function getPurchaseTotals($business_id, $start_date = null, $end_date = null, $location_id = null)
    {
        ...
        $output['total_purchase_exc_tax'] = $purchase_details->sum('final_total');
    ...
    public function getCostTotals($business_id, $start_date = null, $end_date = null, $location_id = null, $created_by = null)
    {

        $query= DB::table('transaction_sell_lines as tslp')
            ->join('variations as v','v.id','=','tslp.variation_id')
            ->join('transactions as t','t.id','=','tslp.transaction_id')
            ->where('t.type','sell')
            ->select(
            DB::raw("((tslp.quantity - tslp.quantity_returned) * v.default_purchase_price) as qty_sell"));
        ...
        $output = $query->get()->sum('qty_sell');

        return $output;
    }


    public function getCostTotals2($business_id, $start_date = null, $end_date = null, $location_id = null, $created_by = null)
    {

        $query= DB::table('transaction_sell_lines_purchase_lines as tslp')
            ->join('purchase_lines as pl','pl.id','=','tslp.purchase_line_id')
            ->join('variations as v','v.id','=','pl.variation_id')
            ->join('transactions as t','t.id','=','pl.transaction_id')
            ->select('pl.id','pl.purchase_price','tslp.quantity',
            DB::raw("((tslp.quantity - tslp.qty_returned) * v.default_purchase_price) as qty_sell"));

        if (!empty($start_date) && !empty($end_date)) {
            $query->whereBetween(DB::raw('date(tslp.created_at)'), [$start_date, $end_date]);
        }
    ...
    public function getOpeningClosingStock($business_id, $date, $location_id, $is_opening = false)
    {
        ...
        $query3 = BankTransactions::where('business_id', $business_id)
        ->where('type', 'minus')
        ->select(
            'balance'
        );
$query2 = BankTransactions::where('business_id', $business_id)
->where('type', 'add')
->select(
'balance'
);
...
$total_internal_expense = $query3->get();
$total_internal_received = $query2->get();


$totalInternalExpense = $total_internal_expense->sum('balance');
$totalInternalReceived = $total_internal_received->sum('balance');
$totalInternalbalance = $total_internal_received->sum('balance') - $total_internal_expense->sum('balance');
        ...
        $details = $query->first();
        $aa = $details->stock;
        return $aa + $totalInternalbalance;
    }
    ...
    public function getTotalInternalBanking($business_id, $location_id, $start_date = null, $end_date = null)
    {
        $query = BankTransactions::where('business_id', $business_id)
                        ->where('type', 'minus')
                        ->select(
                            'balance'
                        );
        ...
        $permitted_locations = auth()->user()->permitted_locations();
        if ($permitted_locations != 'all') {
            $query->whereIn('bank_transactions.business_id', $permitted_locations);
        }
        ...
        //Filter by the location
        if (!empty($location_id)) {
            $query->where('bank_transactions.business_id', $location_id);
        }

        $total_internal_expense = $query->get();
        $total_internal_received = $query2->get();


        $output['total_internal_expense'] = $total_internal_expense->sum('balance');
        $output['total_internal_received'] = $total_internal_received->sum('balance');
        $output['total_internal_balance'] = $total_internal_received->sum('balance') - $total_internal_expense->sum('balance');
