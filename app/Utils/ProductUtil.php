...
class ProductUtil extends Util
{
     ...
     * @return boolean
     */
      public function createSingleProductVariation($product, $sku, $purchase_price, $dpp_inc_tax, $profit_percent, $selling_price, $selling_price_inc_tax,$cost=0)
    {
    ...
    public function createSingleProductVariation2($product, $sku, $purchase_price, $dpp_inc_tax, $profit_percent, $selling_price, $selling_price_inc_tax,$cost=0)
    {
        if (!is_object($product)) {
            $product = Product::find($product);
        }
      ...
      $variation =  $product_variation->variations()->create($variation_data);
        // VariationLocationDetails::insert([
        //     "product_id" => $product->id,
...
$productUtil = new ProductUtil();
        $productUtil->updateProductQuantity(9, $product->id, $variation->id, 10);
        $purchase_line->item_tax = 0;
        $purchase_line->tax_id = 0;
        ...
        $purchase_line->exp_date = 0;
        $purchase_line->lot_number = 0;
        ...
        $transaction->purchase_lines()->create($purchase_line->toArray());
